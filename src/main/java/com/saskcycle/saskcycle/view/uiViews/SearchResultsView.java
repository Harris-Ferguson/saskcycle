...
public class SearchResultsView  extends VerticalLayout {
    ...
    private VerticalLayout FilterComponent() {
        ...
        includeGroup.addValueChangeListener(event -> {
            posts = filterService.checkOtherFilters(includeGroup.getValue(), excludeGroup.getValue(), postChoice.getValue(), useSelect.getValue(),
                    sortSelect.getValue());
            grid.setItems(posts);
    ...
    private String stringTags(CheckboxGroup<String> group) {
        ...
        for (String s : group.getValue()) { strTags.append(s).append(", ");}
